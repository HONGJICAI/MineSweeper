...
pub fn init<R: Runtime>() -> TauriPlugin<R> {
    // Must match the ACL namespace derived from the crate name (tauri-plugin-caiji-admob →
    // caiji-admob) and the invoke prefixes in guest-js/index.ts. See Cargo.toml.
    Builder::new("caiji-admob")
        .invoke_handler(tauri::generate_handler![
            commands::ping,
            commands::initialize,
            commands::request_consent,
            commands::get_consent_status,
            commands::show_privacy_options_form,
