...
pub(crate) async fn request_consent<R: Runtime>(
    ...
    app.google_admob().request_consent(payload)
}

#[command]
pub(crate) async fn get_consent_status<R: Runtime>(app: AppHandle<R>) -> Result<ConsentResponse> {
    app.google_admob().get_consent_status()
}

#[command]
pub(crate) async fn show_privacy_options_form<R: Runtime>(
    app: AppHandle<R>,
) -> Result<ShowPrivacyOptionsResponse> {
    app.google_admob().show_privacy_options_form()
}
