...
export function generateBoard(safeR: number, safeC: number): CellType[][] {
    // Create empty board
...
export function revealCellInPlace(board: CellType[][], r: number, c: number): boolean {
    if (
        r < 0 ||
        r >= ROWS ||
        c < 0 ||
...
export function checkWin(board: CellType[][]): boolean {
    for (let r = 0; r < ROWS; r++) {
        for (let c = 0; c < COLS; c++) {
...
export function countFlaggedAround(board: CellType[][], r: number, c: number): number {
    let count = 0;
    for (let dr = -1; dr <= 1; dr++) {
        for (let dc = -1; dc <= 1; dc++) {
...
export function revealAroundInPlace(board: CellType[][], r: number, c: number): GameStatus {
    for (let dr = -1; dr <= 1; dr++) {
        for (let dc = -1; dc <= 1; dc++) {
            if (dr === 0 && dc === 0) continue;
            const nr = r + dr, nc = c + dc;
...
export function revealAllMinesInPlace(board: CellType[][]) {
    board.forEach((row) => {
        row.forEach((cell) => {
            if (cell.isMine) {
                cell.isRevealed = true;
